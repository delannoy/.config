...
-- completion
vim.opt.completeopt = { -- A comma-separated list of options for Insert mode completion ins-completion.
...
vim.opt.virtualedit = { -- Virtual editing means that the cursor can be positioned where there is no actual character.
...
-- mouse
vim.opt.mouse = '' -- Enables mouse support.

-- rendering
vim.opt.list = true -- List mode: By default, show tabs as ">", trailing spaces as "-", and non-breakable space characters as "+".
vim.opt.listchars = { -- Strings to use in 'list' mode and for the :list command.
...
vim.opt.winblend = 25 -- Enables pseudo-transparency for a floating window. Valid values are in the range of 0 for fully opaque window (disabled) to 100 for fully transparent background.
-- vim.opt.winborder = 'rounded' -- Defines the default border style of floating windows.

-- search/substitute
vim.opt.inccommand =  -- When nonempty, shows the effects of :substitute, :smagic, :snomagic and user commands with the :command-preview flag as you type.
    'split' -- split: Shows the effects of a command incrementally in the buffer, but also shows partial off-screen results in a preview window.
vim.opt.ignorecase = true -- Ignore case in search patterns, cmdline-completion, when searching in the tags file, expr-== and for Insert-mode completion ins-completion.
...
vim.opt.shiftwidth = tabstop -- Number of columns that make up one level of (auto)indentation.
vim.opt.smarttab = true -- When enabled, the <Tab> key will indent by 'shiftwidth' if the cursor is in leading whitespace.
vim.opt.softtabstop = tabstop -- Create soft tab stops, separated by 'softtabstop' number of columns.
vim.opt.tabstop = tabstop -- Defines the column multiple used to display the Horizontal Tab character (ASCII 9);
